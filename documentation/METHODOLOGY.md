...
./.venv/bin/python 05_composition.py # the deliverable
```
...
table, and `05_composition.py` uses that fit. A variant counts in **every year its availability window
...
`data/composition/`, CSV, regenerated by `05_composition.py` in
about 20 seconds.
